  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Handles 409 (already subscribed) or 400 (missing email)
  ...
  return (
    <div className="w-full">
      <section className="w-full h-[111px] bg-[#D10014] px-6 md:px-16 flex items-center justify-between border-b-[3px] border-[#C8A05B]">
        ...
        {/* Right Input Group */}
        <form onSubmit={handleSubscribe} className="flex items-center w-full max-w-[500px]">
          ...
          <button 
            type="submit"
            disabled={loading}
            className="bg-[#C8A05B] hover:bg-[#b58e4d] transition-colors w-[60px] h-[46px] flex items-center justify-center disabled:opacity-70"
          >
            {loading ? (

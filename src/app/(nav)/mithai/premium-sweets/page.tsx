...
// 1. Define the Interface - _id must be string for MongoDB data
interface Product {
  _id: number; 
  name: string;
  price: number;
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        // 2. FILTER: Only include the three specific categories from your image
        ...
        const filtered = data.filter((item: Product) => {
          ...
          // Returns true only if the category name is in our allowed list
      ...
      <div className="flex flex-wrap gap-6">
        {products.map((p) => (
          <ProductCard 
            key={p._id} 
            product={{
              id: p._id, // This is now a string to match your DB
              name: p.name,
